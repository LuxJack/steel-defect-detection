...
- **AI 推理**：YOLOv8（ultralytics）+ OpenCV
...
- **UI 样式**：原生 CSS 仿工业仪表盘，Phosphor Icons
- **图表**：ECharts 5
- **路由 / 状态管理（扩展版）**：Vue Router + Pinia（`src/` 目录备选）
...
- **图片检测**：上传图片 → YOLOv8 推理 → 带标注框的结果图实时回显
- **视频逐帧检测**：SSE 流式推送，前端实时显示原始帧与检测帧双画面及进度条
- **摄像头实时识别**：调用本地摄像头逐帧检测
- **可调阈值**：前端滑块实时调整置信度（Confidence）与交并比（IoU）
...
│   ├── uploads/            # 上传原始文件（运行期自动创建）
│   └── results/            # 带标注的结果图/视频（运行期自动创建）
│
├── frontend/
│   ├── index.html          # 主页面（Vue 3 CDN 单文件，直接使用）
│   ├── js/
...
│
└── database/
    └── steel_defect.db     # SQLite 数据库文件（运行期自动生成）
```
...
将 YOLOv8 权重文件放到 `model/weights/` 目录：

```
model/weights/yolov8n.pt   ← YOLOv8 Nano 基线（需自行下载或已放置）
...
docker run -p 8000:8000 steel-defect-detection
```
...
- 项目路径中如含有中文或空格，`cv2.imwrite` 在 Windows 下会静默失败；本项目已改用 `cv2.imencode` + 二进制写入以兼容中文路径
- `backend/results/` 和 `backend/uploads/` 目录由应用运行期自动创建
